          ...
          <div className='relative z-20 w-full max-w-md transform overflow-hidden rounded-lg bg-white px-6 py-4 shadow-xl transition-all sm:max-w-lg'>
            ...
            <form action={actionUrl} method='post' className='space-y-4'>
              <div>
                ...
                />
                {formData.error?.date && (
                  <p className='text-red-500 text-sm mt-1'>{formData.error.date}</p>
                )}
              </div>
              ...
              <div>
                ...
                {formData.error?.amount && (
                  <p className='text-red-500 text-sm mt-1'>{formData.error.amount}</p>
                )}
              </div>
              ...
              <div>
                ...
                >
                  {categories.contents.map((category) => (
                    <option
                      value={category.id}
                      key={category.id}
                      selected={String(category.id) === formData.income_category_id}
                    >
                      {category.name}
                    </option>
                  ))}
                ...
                {formData.error?.asset_category_id && (
                  <p className='text-red-500 text-sm mt-1'>{formData.error.asset_category_id}</p>
                )}
              </div>
              <div>
                ...
                {formData.error?.description && (
                  <p className='text-red-500 text-sm mt-1'>{formData.error.description}</p>
                )}
              </div>

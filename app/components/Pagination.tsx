...
export const Pagination: FC<Props> = ({ pageSize, hrefPrefix, query, currentPage = 1 }) => {
  ...
  const getPaginationItem = (p: number) => {
    ...
    return (
      <a
        className='flex justify-center items-center h-full text-gray-600 hover:text-gray-400'
        href={getPath(p)}
      >
        {p}
      </a>
  ...
  return (
    <div className='py-4'>
      <ul className='flex flex-wrap justify-center items-center py-10 text-xl font-medium'>
        ...
        {currentPage >= 4 && <li className='list-none w-10 h-10 m-1'>{getPaginationItem(1)}</li>}
        ...
        {currentPage <= pageSize - 3 && (
          <li className='list-none w-10 h-10 m-1'>{getPaginationItem(pageSize)}</li>
        )}

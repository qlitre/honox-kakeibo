...
function initBalanceTransitionChart(
  ...
  const container = canvas.closest('.balance-transition-chart-container') as HTMLElement
  if (!container) return
  ...
  buttons.forEach((btn) => {
    btn.addEventListener('click', () => {
      const mode = (btn as HTMLElement).dataset.mode as 'both' | 'income' | 'expense'
      viewMode = mode
  ...
  function updateDetailDisplay() {
    const detailContainer = container.querySelector('.balance-detail-container') as HTMLElement

        ...
        <Table headers={headers}>
          <tbody className="divide-y divide-gray-200 bg-white">
            {fundTransactions.contents.map((fund_transaction) => (
              <tr
                key={fund_transaction.id}
                className={`${fund_transaction.id === lastUpdateId ? "bg-green-100" : "hover:bg-gray-50"}`}
              >
                ...
                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500 flex space-x-4 justify-center">
                  ...
                  <FundTransactionDeleteModal
                    actionUrl={`/auth/fund_transaction/${fund_transaction.id}/delete?${queryString}`}
